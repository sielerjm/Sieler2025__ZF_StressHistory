# Test script for the simplified neutral model function
# This script tests the simplified neutral model function that avoids MLE issues
...
tryCatch({
  ...
  cat("SUCCESS: Simplified neutral model predictions completed!\n")
  cat("Predictions dimensions:", dim(neutral_predictions), "\n")
  cat("First few predictions:\n")
  print(head(neutral_predictions, 5))
}, error = function(e) {
  cat("ERROR in simplified neutral model predictions:", e$message, "\n")
})

cat("\n=== TEST COMPLETE ===\n")
cat("If both tests passed, the simplified neutral model function is working correctly.\n") 