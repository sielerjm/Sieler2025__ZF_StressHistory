...
prepare_regulation_heatmap <- function(enrichment_results, direction = "up", top_n = 20) {
  ...
  if (nrow(heatmap_data) == 0 || ncol(heatmap_data) == 0) {
    return(list(heatmap_data = NULL, color_scheme = NULL, valid = FALSE))
  }
  ...
  # Check if all values are the same (which causes pheatmap to fail)
  if (length(unique(as.vector(heatmap_data))) <= 1) {
    return(list(heatmap_data = NULL, color_scheme = NULL, valid = FALSE))
  }
  
  return(list(heatmap_data = heatmap_data, color_scheme = color_scheme, valid = TRUE))
}
...
save_enrichment_results <- function(enrichment_results, analysis_name) {
  ...
  for (contrast_name in names(enrichment_results$go_results)) {
    ...
    if (nrow(go_res) > 0) {
      go_filename <- file.path(analysis_dir, paste0(contrast_name, "_GO_enrichment.tsv"))
  ...
  for (contrast_name in names(enrichment_results$kegg_results)) {
    ...
    if (nrow(kegg_res) > 0) {
      kegg_filename <- file.path(analysis_dir, paste0(contrast_name, "_KEGG_enrichment.tsv"))
  ...
  if (length(enrichment_results$go_results) > 0) {
    ...
    go_summary_filename <- file.path(analysis_dir, paste0(analysis_name, "_GO_summary.tsv"))
  ...
  if (length(enrichment_results$kegg_results) > 0) {
    ...
    kegg_summary_filename <- file.path(analysis_dir, paste0(analysis_name, "_KEGG_summary.tsv"))
...
if (length(all_summary_data) > 0) {
  ...
  order_df <- order_df %>%
    group_by(Analysis) %>%
    arrange(desc(Total)) %>%
    mutate(Contrast_ordered = factor(Contrast, levels = unique(Contrast))) %>%
    ungroup()

  # Join back to your long data
  combined_summary_long2 <- combined_summary_long %>%
    left_join(order_df %>% select(Analysis, Contrast, Contrast_ordered), by = c("Analysis", "Contrast"))
...
if (!is.null(enrichment_results[["Parasite_Effect"]])) {
  ...
  if (length(go_results) > 0) {
    ...
    if (go_up_data$valid && !is.null(go_up_data$heatmap_data) && nrow(go_up_data$heatmap_data) > 0) {
      pheatmap::pheatmap(
      ...
      cat("No valid up-regulated GO terms for heatmap visualization.\n")
    }
    
    # Down-regulated GO terms
    go_down_data <- prepare_regulation_heatmap(go_results, "down", top_n = 20)
    if (go_down_data$valid && !is.null(go_down_data$heatmap_data) && nrow(go_down_data$heatmap_data) > 0) {
      pheatmap::pheatmap(
      ...
      cat("No valid down-regulated GO terms for heatmap visualization.\n")
    }
  }
  ...
  if (length(kegg_results) > 0) {
    ...
    if (kegg_up_data$valid && !is.null(kegg_up_data$heatmap_data) && nrow(kegg_up_data$heatmap_data) > 0) {
      pheatmap::pheatmap(
      ...
      cat("No valid up-regulated KEGG pathways for heatmap visualization.\n")
    }
    
    # Down-regulated KEGG pathways
    kegg_down_data <- prepare_regulation_heatmap(kegg_results, "down", top_n = 20)
    if (kegg_down_data$valid && !is.null(kegg_down_data$heatmap_data) && nrow(kegg_down_data$heatmap_data) > 0) {
      pheatmap::pheatmap(
      ...
      cat("No valid down-regulated KEGG pathways for heatmap visualization.\n")
    }
  }
...
if (length(enrichment_results) > 0) {
  ...
  create_enrichment_summary <- function(enrichment_list, analysis_name) {
    ...
    go_summary <- purrr::map_dfr(enrichment_list$go_results, ~ 
      dplyr::slice_max(.x, order_by = log_padj, n = 5), 
      .id = "contrast"
    ) %>%
  ...
  # Create summaries for each analysis
